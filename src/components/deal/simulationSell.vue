          ...
          <div class="content">
            <div class="grey">
              <div class="Purchase">
                ...
                <span>开仓&nbsp;&nbsp;{{item.openPrice}}</span>
                <span>现价&nbsp;&nbsp;{{nowPriceArr[item.code]}}</span>
              </div>
              <div class="presentPrice">
              <span
                :class="color(item.direction)">{{item.direction, item.openPrice, nowPriceArr[item.code], item.exchangeRate, item.code
                ,item.number,item.pointVlaue,item.waveMin | exchangeRate2}}</span>
                <span>止盈&nbsp;&nbsp;{{item.profitStopPrice}}</span>
                <span>止损&nbsp;&nbsp;{{item.loseStopPrice}}</span>
              </div>
            </div>
            <div class="right" @click="unravel(item)">
              <span>平仓</span>
            </div>
          </div>
        </li>
        ...
        <div class="swalCenter">
          <p class="item1">
            到达&nbsp;<span class="minus-btn"
                          @click="profitDown(profitStopPrice,profitStopMaxPrice,loseStopMinPrice,waveMin)">-</span><input
            class="num-input"
            v-model="profitStopPrice" ><span class="add-btn"
                                             @click="profitUp(profitStopPrice,profitStopMaxPrice,loseStopMinPrice,waveMin)">+</span>&nbsp;&nbsp;点位时止盈平仓
          </p>
          <p>
            到达&nbsp;<span class="minus-btn"
                          @click="lastDown(loseStopPrice,profitStopPrice,loseStopMinPrice,waveMin)">-</span><input
            class="num-input"
            v-model="loseStopPrice" ><span class="add-btn"
                                           @click="lastUp(loseStopPrice,profitStopPrice,loseStopMinPrice,waveMin)">+</span>&nbsp;&nbsp;点位时止损平仓
          </p>
        </div>
        <div class="swalbottom">
          <span @click="cancel">取消</span>
  ...
  export default {
    ...
    filters: {
      ...
      exchangeRate2(direction, openPrice, settlePrice, exchangeRate, code,number,pointVlaue,w) {
        ...
        console.log(direction, openPrice, settlePrice, exchangeRate, code,number,pointVlaue,w);
    ...
    methods: {
      dealSellFn() {
        ...
        this.getNowPrice();
        var self = this;
        window.dealSellTime = setInterval(function(){
          self.getNowPrice();
        },1000);
      },
      toogleBtn() {
      ...
      toSet(item) {
        ...
        this.openPrice = item.openPrice; //开仓价格
...
//止盈平仓减去 -
      ...
      averageSure(direction) {   //止损平仓确认
        if(direction == 1){
          ...
          if(this.profitStopPrice < this.openPrice){
            this.profitStopPrice = this.openPrice;
            toast('止盈价格不能小于' + this.openPrice);
            return false;
          }
          ...
          if(this.loseStopPrice > this.openPrice){
            this.loseStopPrice = this.openPrice;
            toast('止损价格不能大于' + this.openPrice);
            return false;
          }
        }else {
          if (this.profitStopPrice < this.profitStopMaxPrice) {
...
//     平仓确定时间
      ...
      getList() {
        ...
        //我的跟单
        home.getMNCancel(senddata).then((res) => {
          if (res.data.code == 200) {
      ...
      getNowPrice(){
        ...
        var self = this;
        home.getMNNowPrice(senddata2).then((res) => {
          if (res.data.code == 200) {
